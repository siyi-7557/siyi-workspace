...
> 💡 **小提示**：如果希望获得**最佳体验**，可以按需配置对应的 API Key（在 `clients/personal-ai/.env`，复制 `.env.example` 后填写即可，不会入库）。**即便不配置，整套演示也完全可以跑通**——只是部分能力会以更「轻量」的方式呈现：
>
> | Key | 用途 | 未配置时的表现 | 配置后 |
> |---|---|---|---|
> | `ZHIPU_API_KEY` | 聊天 / 回答（智谱 GLM） | 自动用离线 MockLLM 占位回答（闭环同样清晰可见） | 真实模型，回答更自然 |
> | `SILICONFLOW_API_KEY` | 语义向量检索（BGE-m3） | 回退为纯 BM25（召回略有下降） | 真语义向量，召回质量更佳 |
>
> 两者来自**不同的服务商**，需要分别申请。如果您想看到**完整而真实**的效果，把两个都配上会是更好的选择 ✨
