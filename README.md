...
> **⚠️ 想获得「最佳效果」，请自行配置对应的 API Key**（都在 `clients/personal-ai/.env`，复制 `.env.example` 后填写，不会入库；**不配也能跑演示，但效果会降级**）：
>
> | Key | 用途 | 不配置时的降级 | 配置后 |
> |---|---|---|---|
> | `ZHIPU_API_KEY` | 聊天 / 回答（智谱 GLM） | 自动用**离线 MockLLM 占位回答**（闭环能看，但非真实模型） | 真实模型、自然回答、工具调用 |
> | `SILICONFLOW_API_KEY` | 语义向量检索（BGE-m3） | 回退**纯 BM25**（召回略降） | **真语义向量**，混合检索召回更好 |
>
> 两个是不同的服务商，**需分别申请、不能一个顶两个**；想体验**完整真实效果**，建议**两个都配**。
