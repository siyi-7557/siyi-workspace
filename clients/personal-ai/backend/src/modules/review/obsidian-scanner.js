...
/**
 * 按最近 N 天重算复盘统计（用于时间范围筛选）
 * @param {number|null} days - 最近天数，null/0 表示全部
 */
function computeStatsForRange(days = null) {
  const result = scanReviews({ page: 1, pageSize: 1000 });
  const reviews = filterReviewsByDays(result.reviews, days);
  return { stats: calculateStats(reviews), total: reviews.length };
}
...
function getAllActionItems(options = {}) {
  ...
  // 合并手动创建的行动项（custom_ 前缀，前端据此显示删除按钮）
  const manualItems = loadManualActionItems().map(item => ({
    id: item.id,
    ...
    sourceReview: '手动创建',
    sourcePath: null,
    reviewDate: item.createdAt ? item.createdAt.slice(0, 10) : '',
    reviewScore: null,
    status: item.status || 'pending',
    statusUpdatedAt: item.updatedAt || null,
    note: item.note || '',
    dueDate: item.dueDate || null
  }));
  allItems = allItems.concat(manualItems);
  ...
  const stats = {
    ...
    byType: {
      ...
      long_term: allItems.filter(i => i.type === 'long_term').length,
...
/**
 * 行动项状态存储文件路径（使用 backend/data，避免沙箱 %APPDATA% 写限制）
 */
function getActionItemStatusPath() {
  return path.join(config.DATA_DIR, 'action-items-status.json');
}

/**
 * 旧版行动项状态文件路径（%APPDATA%，用于一次性迁移）
 */
function getLegacyActionItemStatusPath() {
  return path.join(require('os').homedir(), 'AppData', 'Roaming', 'siyi-workbench', 'action-items-status.json');
}

/**
 * 迁移旧位置（%APPDATA%）的行动项状态文件到 backend/data
 */
function migrateActionItemStatus() {
  const newPath = getActionItemStatusPath();
  if (fs.existsSync(newPath)) return;
  const legacyPath = getLegacyActionItemStatusPath();
  if (fs.existsSync(legacyPath)) {
    try {
      fs.mkdirSync(config.DATA_DIR, { recursive: true });
      fs.copyFileSync(legacyPath, newPath);
      console.log('[ObsidianScanner] 已迁移行动项状态文件到:', newPath);
    } catch (err) {
      console.warn('[ObsidianScanner] 迁移行动项状态文件失败:', err.message);
    }
  }
}
...
function updateActionItemStatus(id, status, note = '', dueDate = null) {
  ...
  const ok = saveActionItemStatus(statusStore);
  // 手动创建的行动项：同步更新其自身状态字段，保证列表读取一致
  if (id && id.startsWith('custom_')) {
    const items = loadManualActionItems();
    const item = items.find(i => i.id === id);
    if (item) {
      item.status = status;
      if (note) item.note = note;
      if (dueDate !== null) item.dueDate = dueDate;
      item.updatedAt = statusStore[id].updatedAt;
      saveManualActionItems(items);
    }
  }
  return ok;
}
...
/**
 * 手动创建的行动项存储文件路径（使用 backend/data，避免沙箱 %APPDATA% 写限制）
 */
function getManualActionItemsPath() {
  return path.join(config.DATA_DIR, 'manual-action-items.json');
}

/**
 * 加载手动创建的行动项
 */
function loadManualActionItems() {
  try {
    const filePath = getManualActionItemsPath();
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf-8');
    ...
    console.warn('[ObsidianScanner] 加载手动行动项失败:', err.message);
  }
  return [];
}
 ...
 * 保存手动创建的行动项
 */
function saveManualActionItems(items) {
  try {
    const filePath = getManualActionItemsPath();
    const dir = path.dirname(filePath);
    if (!fs.existsSync(dir)) {
    ...
    console.error('[ObsidianScanner] 保存手动行动项失败:', err.message);
    return false;
  }
}
...
/**
 * 创建手动行动项
 * @param {Object} data - { action, priority, type, dueDate, note }
 * @returns {Object} 结果
 */
function createActionItem(data = {}) {
  const action = (data.action || '').trim();
  if (!action) {
    return { success: false, error: '行动项内容不能为空' };
  }

  const priority = ['P0', 'P1', 'P2'].includes(data.priority) ? data.priority : 'P2';
  const type = ['immediate', 'short_term', 'long_term', 'note'].includes(data.type) ? data.type : 'short_term';
  const now = new Date();
  const id = `custom_${now.getTime()}_${Math.random().toString(36).slice(2, 7)}`;
  ...
  if (!saveManualActionItems(items)) {
...
/**
 * 删除手动行动项
 * @param {string} id - 行动项ID（custom_ 前缀）
 */
function deleteActionItem(id) {
  ...
  return saveManualActionItems(filtered);
}
 ...
 */
// 按最近 N 天过滤复盘列表（兼容 stats/patterns/practices 时间范围统计）
...
function analyzePatterns(days = null) {
  const result = scanReviews({ page: 1, pageSize: 1000, sortBy: 'date', sortOrder: 'desc' });
  let reviews = filterReviewsByDays(result.reviews, days);
...
/**
 * 跨会话好做法排行（按标题聚合出现次数）
 * @returns {Object} 好做法排行数据
 */
function analyzeGoodPractices(days = null) {
  const result = scanReviews({ page: 1, pageSize: 1000, sortBy: 'date', sortOrder: 'desc' });
  let reviews = filterReviewsByDays(result.reviews, days);
  ...
  reviews.forEach(review => {
    try {
      const detail = getReviewDetail(review.path);
      if (!detail) return;
      const practices = extractGoodPracticesFromContent(detail.content);
      practices.forEach(p => {
        allPractices.push({
          ...p,
          sourceReview: review.title,
      ...
      console.warn(`[ObsidianScanner] 提取好做法失败 ${review.path}:`, err.message);
    }
  });
  ...
  allPractices.forEach(p => {
    ...
    if (!title) return;
    if (!practiceStats[title]) {
      practiceStats[title] = { title, description: p.description || '', reusability: p.reusability || '', count: 0, practices: [] };
    }
    practiceStats[title].count++;
    practiceStats[title].practices.push(p);
    if (!practiceStats[title].description && p.description) practiceStats[title].description = p.description;
  });

  const topPractices = Object.values(practiceStats)
...
function extractAbilityScores(content) {
  ...
  // 板块3正文（extractSectionContent 遇到 ### 子标题会截断，仅用于同行格式兜底）
  const section3 = extractSectionContent(content, [/板块3/, /用户行为分析/, /行为分析/]);
  const inlineScope = section3 || content;

  // 格式A：维度名与分数同行，如 目标清晰度: 4分 / **目标清晰度**: 4/5 / 表格行 / 列表行
  ABILITY_DIMENSIONS.forEach(dim => {
    ...
    for (const pattern of inlinePatterns) {
      const match = inlineScope.match(pattern);
      if (match) {
        const v = parseInt(match[1]);
        if (v >= 1 && v <= 5) { scores[dim.id] = v; break; }
      }
    }

    // 格式B：小节标题「### 3.1 目标清晰度」后（可跨空行）紧跟「**评分：4 分**」，全文匹配
    if (!scores[dim.id]) {
      const escapedName = dim.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const headingRe = new RegExp(`#{2,4}\\s*[\\d.]*\\s*${escapedName}[\\s\\S]{0,200}?\\*{0,2}\\s*评分[：:]\\s*(\\d)\\s*分?`, 'i');
      const hm = content.match(headingRe);
      if (hm) {
        const v = parseInt(hm[1]);
        if (v >= 1 && v <= 5) scores[dim.id] = v;
      }
    }

    // 格式C：小节标题内嵌评分，如「### 3.1 目标清晰度：4分」/「### 3.1 目标清晰度 4分」，全文匹配
    if (!scores[dim.id]) {
      const escapedName = dim.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const headingScoreRe = new RegExp(`#{2,4}\\s*\\d+(?:\\.\\d+)?\\s*${escapedName}\\s*[：:]?\\s*(\\d)\\s*分?`, 'i');
      const hsc = content.match(headingScoreRe);
      if (hsc) {
        const v = parseInt(hsc[1]);
        if (v >= 1 && v <= 5) scores[dim.id] = v;
      }
    }
  });
...
function getPeriodData(startDate, endDate) {
  const result = scanReviews({ page: 1, pageSize: 1000, sortBy: 'date', sortOrder: 'asc' });
  const reviews = result.reviews || [];
  const periodReviews = reviews.filter(r => r.date && r.date >= startDate && r.date <= endDate);

  if (periodReviews.length === 0) {
    return { startDate, endDate, total: 0, message: '该周期内没有复盘记录' };
  }

  const stats = calculateStats(periodReviews);
  const allProblems = [];
  const allLearnings = [];
  const allActionItems = [];
  ...
  periodReviews.forEach(review => {
    try {
      ...
      if (!detail) return;
      const problems = extractProblemsFromContent(detail.content);
      problems.forEach(p => { p.reviewDate = review.date; p.reviewTitle = review.title; allProblems.push(p); });
      const learnings = extractLearningsFromContent(detail.content);
      learnings.forEach(l => { l.reviewDate = review.date; l.reviewTitle = review.title; allLearnings.push(l); });
      const actions = extractActionItemsFromContent(detail.content);
      actions.forEach(a => { a.reviewDate = review.date; a.reviewTitle = review.title; allActionItems.push(a); });
    } catch (err) {
      console.warn(`[ObsidianScanner] 提取周期数据失败 ${review.path}:`, err.message);
    }
  });

  const patternStats = {};
  allProblems.forEach(p => {
    matchProblemPattern(p).forEach(matched => {
      if (!patternStats[matched.id]) patternStats[matched.id] = { name: matched.name, count: 0, wastedTurns: 0 };
      patternStats[matched.id].count++;
      patternStats[matched.id].wastedTurns += p.wastedTurns || 0;
    });
  });

  const abilityTrend = periodReviews.map(review => {
    try {
      const detail = getReviewDetail(review.path);
      if (!detail) return null;
      const scores = extractAbilityScores(detail.content);
      if (!Object.values(scores).some(s => s > 0)) return null;
      const avg = Math.round((Object.values(scores).reduce((a, b) => a + b, 0) / 6) * 10) / 10;
      return { date: review.date, title: review.title, scores, avg };
  ...
  return {
    startDate, endDate, total: periodReviews.length, stats,
    reviews: periodReviews.map(r => ({ date: r.date, title: r.title, score: r.average_score, problems: r.total_problems, goodPractices: r.total_good_practices, prompts: r.total_prompts, source: r.source })),
    problems: allProblems, learnings: allLearnings, actionItems: allActionItems,
    patternStats: Object.entries(patternStats).sort((a, b) => b[1].count - a[1].count).map(([id, data]) => ({ id, ...data })),
    abilityTrend
  };
}
 ...
 */
async function generatePeriodReport(period = 'month', startDate, endDate) {
  let reportId = null;
  // 支持自定义日期范围：若传入明确的起止日期则直接使用，否则按周期基于今天计算
  const isCustom = !!(startDate && endDate);
  if (isCustom) {
    // 使用调用方传入的 startDate/endDate，无需重新计算
  } else {
    ({ startDate, endDate } = getPeriodDateRange(period));
  }
  const periodData = getPeriodData(startDate, endDate);

  if (periodData.total === 0) {
    return { success: false, error: `${startDate} 至 ${endDate} 没有复盘记录，无法生成报告`, data: periodData };
  }

  try {
    const aiClient = require('../../core/ai/client');
    const labels = { week: '周', month: '月', quarter: '季度', year: '年' };
    // 自定义日期范围时用通用称呼，避免"本月/本周"等措辞与所选期间不符
    const label = isCustom ? '周期' : (labels[period] || '周期');

    const inputData = {
      周期: `${startDate} 至 ${endDate}`,
      复盘数量: periodData.total,
      平均评分: periodData.stats.avgScore,
      总问题数: periodData.stats.totalProblems,
      好做法数: periodData.stats.totalGoodPractices,
      可复用Prompt数: periodData.stats.totalPrompts,
      完成率: periodData.stats.completionRate + '%',
      问题模式TOP3: periodData.patternStats.slice(0, 3).map(p => ({ 模式: p.name, 出现次数: p.count, 浪费轮次: p.wastedTurns })),
      学习收获TOP5: periodData.learnings.slice(0, 5).map(l => ({ 标题: l.title, 类型: l.type, 掌握程度: l.mastery })),
      行动项TOP5: periodData.actionItems.slice(0, 5).map(a => ({ 优先级: a.priority, 行动: (a.action || '').substring(0, 100) })),
      能力评分趋势: periodData.abilityTrend.map(t => ({ 日期: t.date, 平均分: t.avg }))
    };

    const systemPrompt = `你是一个专业的AI能力成长教练和数据分析师。根据用户提供的${label}度复盘数据，生成一份深度、全面、可执行的${label}度成长报告。

报告核心要求：
1. 数据驱动：每个结论必须有具体数据支撑，禁止空洞套话
2. 深度分析：不仅描述现象，更要分析根本原因和趋势
3. 具体案例：引用具体的复盘记录作为证据，增强说服力
4. 可执行性：改进建议必须具体、可衡量、有优先级
5. 客观专业：既肯定亮点，也直面问题，不回避不足

报告结构（严格按照以下格式输出Markdown，每个部分都要充分展开）：

## 📊 本${label}概览
- 用3-5句话总结本${label}整体表现和核心变化
- 关键数据仪表盘：复盘数、平均分、问题数、好做法数、Prompt数、完成率
- 与上${label}对比（如果有数据）：进步了什么？退步了什么？

## 📈 能力成长深度分析
- 各能力维度得分对比（如果有能力评分数据）
- 进步最大的方面：具体数据 + 原因分析 + 可复用经验
- 需要加强的方面：具体数据 + 根本原因分析 + 改进路径
- 能力发展趋势：是上升、平稳还是下降？背后的原因是什么？

## ⚠️ 主要问题与根本原因分析
- TOP3问题模式：每个问题包含【现象描述】【出现次数】【浪费轮次】【根本原因分析】【具体改进建议】
- 问题之间的关联性：这些问题是否有共同的根源？
- 问题的严重程度排序：哪些问题最影响效率？哪些最容易解决？

## ✅ 亮点与可复用经验
- 本${label}做得最好的3件事：具体案例 + 为什么有效 + 如何持续
- 重要学习收获TOP5：每个收获包含【内容】【掌握程度】【如何应用到下一${label}】
- 可复用Prompt/方法论：哪些做法可以沉淀为标准流程？
- 好做法的共性：这些亮点背后有什么共同的成功因素？

## 🎯 具体复盘案例分析
- 选取2-3篇有代表性的复盘进行深度分析
- 每篇包含：【复盘标题】【日期】【评分】【核心问题】【亮点做法】【可借鉴经验】
- 这些案例对整体成长的启示是什么？

## 🚀 下${label}改进行动计划
- 5个具体的、可执行的改进目标，按优先级排序
- 每个目标包含：【目标描述】【衡量标准】【具体行动】【预期效果】【优先级】
- 短期（1-2周）可以立即做的事
- 中期（1${label}）需要持续推进的事
- 如何避免本${label}的问题在下${label}重复出现？

直接输出Markdown格式的完整报告，不要额外解释，不要省略任何部分。`;

    const userMessage = `${label}度复盘数据：
${JSON.stringify(inputData, null, 2)}

请根据以上数据生成${label}度成长报告。`;
    ...
    try {
      ...
      const summary = (reportContent || '').replace(/[#*>`\-\n]/g, ' ').substring(0, 200);
      const result = db.execute(
        `INSERT INTO period_reports (period, start_date, end_date, title, summary, content, review_count, avg_score, total_problems) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [period, startDate, endDate, `${startDate} 至 ${endDate} 成长报告`, summary, reportContent, periodData.total, periodData.stats.avgScore || 0, periodData.stats.totalProblems || 0]
      );
      reportId = result.lastInsertRowid;
    } catch (dbErr) {
      console.warn('[ObsidianScanner] 报告存储到数据库失败:', dbErr.message);
    }

    return { success: true, id: reportId, period, startDate, endDate, content: reportContent, data: periodData, generatedAt: new Date().toISOString() };
  } catch (err) {
    console.error('[ObsidianScanner] 生成周期报告失败:', err.message);
    return { success: false, error: `AI生成失败: ${err.message}`, data: periodData };
  }
}
 ...
 * 获取报告列表
 */
function getReportList(limit = 50) {
  try {
    const db = require('../../core/db');
    return db.query(
      `SELECT id, period, start_date, end_date, title, summary, review_count, avg_score, total_problems, generated_at FROM period_reports ORDER BY generated_at DESC LIMIT ?`,
      [limit]
    );
  } catch (err) {
    console.error('[ObsidianScanner] 获取报告列表失败:', err.message);
    return [];
  }
}
...
function getReportDetail(id) {
  try {
    const db = require('../../core/db');
    return db.queryOne(
      `SELECT * FROM period_reports WHERE id = ?`,
      [id]
    );
  } catch (err) {
    console.error('[ObsidianScanner] 获取报告详情失败:', err.message);
    return null;
  }
}
function getReportDetail(id) {
...
function deleteReport(id) {
  try {
    const db = require('../../core/db');
    const result = db.execute(`DELETE FROM period_reports WHERE id = ?`, [id]);
    return result.changes > 0;
  } catch (err) {
    console.error('[ObsidianScanner] 删除报告失败:', err.message);
    return false;
  }
}
